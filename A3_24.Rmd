...
#count
n_distinct(df$PatID)
  length(which(unique(df$PatID) & df$Gender=='M'))
...
#plot
ggplot()+
  geom_bar(aes(df$Gender),fill=df$Diagnosis)
  facet_wrap(~Diagnosis)
  geom_density(aes(df$PatID), fill=df$Diagnosis, alpha = 0.3)+
  geom_density(aes(df$PatID), fill=df$Gender, alpha=0.3)+
  # xlim(0,1000)+
  # ylim(0,1000)+
...
#FA
library(psych,fmsb,GPArotation)
df_fil<-df[,8:398] #filter for the factors
df_cor <- cor(df_fil)
heatmap(df_cor)
...
ggsave('fa.png',nine,width=40,height=40,units="in")

#save laoding with diagnosis
load <- fa_max[["loadings"]]

regdata <- cbind(df[1:3], fa_max$scores)
#Labeling the data

#names(regdata) <- c("Diagnosis", "M1", "M2",
...
#Noise
rec_em <- train_df %>%
  recipe(Diagnosis ~ MR1+MR2+MR3+MR4+MR5+MR6+MR7+MR8+MR9,data=train_df) %>% # defines the outcome
  update_role(train_df$NewID,new_role = "id variable") %>% 
...
test_s <- bake(rec_em, new_data = test_df, all_numeric_predictors()) %>% 
...
#get prior
view(get_prior(em, data=train_df,family = bernoulli))
ep <- c(
  prior(normal(0,1),class=b),
  prior(normal(0,1),class=Intercept),
...
model_em_s <- 
  brm(
    ...
    cores = 2,#more cores, more parallel processing
    chains = 2, #at least two chains, use 4
    file = "empirical_prior scaled",
    control = list(adapt_delta = 0.99, max_treedepth = 20))
  pp_check(model_em, ndraws = 100)+labs(title='prior empirical')+xlim(-0.5,1.5)
...
#Error in seq_len(n) : argument must be coercible to non-negative integer
 #sd <- 
  ggplot()+
  geom_density(aes(pos_em$sd_NewID__Intercept),
                   fill='green',
                   color='green',
                   alpha=0.3
                   )+
  geom_density(aes(pos_em$prior_sd_NewID),
                   fill='red',
                   color='red',
                   alpha=0.3
                   )+
...
pop_em <- grid.arrange(variables,sd,intercept, top='Prior-posterior Empirical',nrow=1)

ggsave("prior_posterior_em.PNG",pop_em,height=7,width=10,units="in")
...
train_df <- train_df %>% 
 ...
 train_df$PredictionsPerc <- predict(fit_em)[, 2]
 train_df$Predictions[train_df$PredictionsPerc > 0.5] <- "SCZ"
 train_df$Predictions[train_df$PredictionsPerc <= 0.5] <- "CT"
...
##Test
#fit_em_test <- update(fit_em,newdata=test_df)
test_df <- test_df %>% 
  mutate(PredictionsPerc=NA) %>% 
  mutate(Predictions=NA)

test_df$PredictionsPerc <- predict(fit_em, newdata=test_df, allow_new_levels=TRUE)[, 2]
#Error: The following variables are missing in the draws object: {'b_PredictionsPercTRUE'}
test_df$PredictionsPerc <- predict(fit_em_test)[, 2]
test_df$Predictions[test_df$PredictionsPerc > 0.5] <- "SCZ"
test_df$Predictions[test_df$PredictionsPerc <= 0.5] <- "CT"
