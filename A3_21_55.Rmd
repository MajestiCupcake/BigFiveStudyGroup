...
df_fil<-df[,8:398] #filter for the factors aka questions
df_cor <- cor(df_fil)
heatmap(df_cor)
...
# train the model
model <- train(df$Diagnosis~., data=df, method="regLogistic", preProcess="scale", trControl=control)
...
#should it be updated with the update()
...
```{r feature importance for empirical data, where to put it?}
pacman::p_load(DALEX,DALEXtra,kernlab,xgboost,knitr,dotwhisker)
